describe 'About Arrays', ->
  ...
  it "should know about destructuring assignment", ->
    #Coffeescript does destructuring assingment, but
    #syntax is a little different than ruby
    [a, b, c] = [1, 2, 3]
    expect(a).toEqual(FILL_ME_IN)
    expect(b).toEqual(FILL_ME_IN)
    expect(c).toEqual(FILL_ME_IN)
    
    # you don't have to use the entire array
    [d, e] = [1, 2, 3]
    expect(e).toEqual(FILL_ME_IN)
    
    # but you can deal with leftovers :)
    [f, g...] = [1, 2, 3]
    expect(g).toEqual(FILL_ME_IN)
